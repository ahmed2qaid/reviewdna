...
function candidate(rule:EngineeringRule):AutomationOpportunity|undefined{
  ...
  const securitySignal=/\b(secrets?|tokens?|credentials?|passwords?|api keys?|bearer)\b/.test(text);
  ...
  if(rule.category==='security'||securitySignal)return make('security-scan',securitySignal&&rule.confidence>=75?'high':'medium','Recurring security guidance may be partially enforceable with scanning or static policy checks.',['secret scanning','SAST or custom security lint','CI security policy check']);
