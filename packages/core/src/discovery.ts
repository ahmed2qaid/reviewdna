...
import { acceptedEvidenceWeight, evidenceDisposition, rejectedEvidenceWeight } from './evidence.js';
import { applyRuleEvolution } from './evolution.js';
...
function scoreCluster(cluster:ClassifiedReview[],now=new Date()):EngineeringRule['scoreBreakdown']{
  const evidence=cluster.length,reviewers=new Set(cluster.map(r=>r.reviewer)).size,dates=cluster.map(r=>new Date(r.createdAt).getTime()).sort((a,b)=>a-b),newest=dates.at(-1)??now.getTime(),oldest=dates[0]??newest,ageDays=Math.max(0,(now.getTime()-newest)/86400000),spanDays=Math.max(0,(newest-oldest)/86400000);
  const acceptedWeight=cluster.reduce((sum,r)=>sum+acceptedEvidenceWeight(r),0),rejectedWeight=cluster.reduce((sum,r)=>sum+rejectedEvidenceWeight(r),0);
  const frequency=Math.min(30,evidence*4),reviewerDiversity=Math.min(20,reviewers*4),recency=Math.max(0,Math.round(20-ageDays/30)),acceptedEvidence=evidence?Math.round(20*acceptedWeight/evidence):0,rejectedEvidencePenalty=evidence?Math.min(20,Math.round(20*rejectedWeight/evidence)):0,persistence=Math.min(10,Math.round(spanDays/30)),conflictPenalty=0,total=Math.max(0,Math.min(100,frequency+reviewerDiversity+recency+acceptedEvidence+persistence-rejectedEvidencePenalty));
  return {frequency,reviewerDiversity,recency,acceptedEvidence,persistence,codeOwnerEvidence:0,rejectedEvidencePenalty,conflictPenalty,total};
}
...
function evidenceOf(r:ClassifiedReview):RuleEvidence{const e:RuleEvidence={id:r.id,prNumber:r.prNumber,reviewer:r.reviewer,createdAt:r.createdAt,url:r.url,body:r.body,disposition:evidenceDisposition(r)};if(r.path!==undefined)e.path=r.path;if(r.accepted!==undefined)e.accepted=r.accepted;if(r.resolved!==undefined)e.resolved=r.resolved;if(r.changedAfterReview!==undefined)e.changedAfterReview=r.changedAfterReview;if(r.deepEvidenceChecked!==undefined)e.deepEvidenceChecked=r.deepEvidenceChecked;if(r.explicitResponse!==undefined)e.explicitResponse=r.explicitResponse;return e;}
function negative(text:string){return /\b(avoid|never|do not|don't|remove|stop|instead of)\b/i.test(text);}
export function detectConflicts(rules:EngineeringRule[]):void{for(let i=0;i<rules.length;i++)for(let j=i+1;j<rules.length;j++){const a=rules[i],b=rules[j];if(!a||!b||a.category!==b.category)continue;if(a.status==='superseded'||b.status==='superseded')continue;const historical=a.relationships?.supersededByFingerprint===b.fingerprint||b.relationships?.supersededByFingerprint===a.fingerprint;if(historical)continue;if(negative(a.text)!==negative(b.text)&&similarity(a.text,b.text)>=.22){a.conflictingRuleIds.push(b.id);b.conflictingRuleIds.push(a.id);a.status=b.status='disputed';for(const r of[a,b]){r.scoreBreakdown.conflictPenalty=15;r.scoreBreakdown.total=Math.max(0,r.scoreBreakdown.total-15);r.confidence=r.scoreBreakdown.total;}}}}
...
export function buildAnalysisFromClusters(records:ReviewRecord[],repository:string,source:'github'|'fixture',clusters:ClassifiedReview[][],initialRejected:RejectedReview[],options:DiscoveryOptions={},metadata:AnalysisBuildMetadata={}):AnalysisResult{
  ...
  const evolution=applyRuleEvolution(rules);
  detectConflicts(rules);
  const prs=new Set(records.map(r=>r.prNumber)),reviewers=new Set(records.map(r=>r.reviewer)),categoryCounts:Record<string,number>={};for(const r of rules)categoryCounts[r.category]=(categoryCounts[r.category]??0)+1;
  return {schemaVersion:'1.0',summary:{repository,generatedAt:new Date().toISOString(),reviewsAnalyzed:records.length,pullRequests:prs.size,reviewers:reviewers.size,rules:rules.length,highConfidenceRules:rules.filter(r=>r.confidence>=80).length,emergingRules:rules.filter(r=>r.status==='emerging').length,conflictingRules:rules.filter(r=>r.conflictingRuleIds.length).length,staleRules:rules.filter(r=>r.status==='stale').length,undocumentedRules:rules.filter(r=>!r.documented).length,documentationCoverage:0,documentationDrift:0,parentRules:evolution.parentRules,childRules:evolution.childRules,supersededRules:evolution.supersededRules,categoryCounts},rules,rejected,metadata:{engineVersion:'0.2.0-dev',mode:'deterministic',source,clusterer:metadata.clusterer??'deterministic',evolutionModel:'relationships-v1',...(metadata.embeddingProvider?{embeddingProvider:metadata.embeddingProvider}:{}),...(metadata.semanticThreshold!==undefined?{semanticThreshold:metadata.semanticThreshold}:{})}};
}
