...
export class GitHubCollector {
  ...
  async collectDocumentation(repository:string):Promise<Array<{path:string;content:string}>>{const[owner,repo]=repository.split('/');if(!owner||!repo)throw new Error('Repository must be in owner/name form.');const paths=['AGENTS.md','CLAUDE.md','CONTRIBUTING.md','.github/copilot-instructions.md','.cursorrules','.cursor/rules/repository.mdc'],out:Array<{path:string;content:string}>=[];for(const path of paths){try{const data=await this.get<{type:string;content?:string;encoding?:string}>(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`);if(data.type==='file'&&data.content&&data.encoding==='base64'){const bytes=Uint8Array.from(atob(data.content.replace(/\n/g,'')),c=>c.charCodeAt(0));out.push({path,content:new TextDecoder().decode(bytes)});}}catch(err){if(!(err instanceof Error)||!err.message.includes('404'))throw err;}}return out;}
}
